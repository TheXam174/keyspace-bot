...
def is_valid_key(line):
    ...
    if len(line) > 200:
        return False
    bad = ["<html", "404", "not found", "error", "<script", "apache"]
                ...
                if st=="succeeded":
                    if tp=="one_time":
                        if goods.get(cat):
                            ...
                            bot.send_message(uid, "⚠️ Ключи по выбранной категории закончились.")
                    else:
            ...
            for cat, data in cats.items():
                if now - data["last_time"] >= 30*86400:
                    if goods.get(cat):
                        ...
                        bot.send_message(int(uid), f"⚠️ Ключи по подписке {cat} закончились.")
        time.sleep(3600)
