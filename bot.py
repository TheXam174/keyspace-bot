...
def is_valid_key(line):
    ...
    if not line or len(line) > 200:
        return False
    bad_words = ["<html", "404", "not found", "error", "<script", "DOCTYPE", "Server at", "nginx", "Apache"]
    return not any(bad in line.lower() for bad in bad_words)

def clean_goods():
    for cat in goods:
        goods[cat] = [k for k in goods[cat] if is_valid_key(k)]
                ...
                if st == "succeeded":
                    ...
                    if tp == "one_time":
                        if goods.get(cat):
                            ...
                            bot.send_message(uid, "❌ Ключи закончились.")
                    else:
