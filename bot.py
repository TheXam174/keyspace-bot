...
def is_valid_key(line):
    ...
    if len(line) > 200 or len(line) < 10:
        return False
    ...
    # Запрещённые слова и фразы, которые означают ошибку или страницу сервера
    bad_words = [
        "404", "not found", "error", "script", "apache", "server", "port",
        "forbidden", "access denied", "proxy", "html", "bad request", "unauthorized",
    ...
    # Проверка на количество спецсимволов — не более 50% от длины
    non_alnum = sum(1 for c in line if not c.isalnum())
    if non_alnum > len(line) / 2:
        return False

    # Дополнительная проверка: ключ обычно не должен содержать повторяющиеся символы больше 4 раз подряд
