...
def is_valid_key(line):
    ...
    if not line or len(line) > 200:
        return False
    bad = ["<html", "404", "not found", "error", "<script", "<!doctype", "apache", "server at", "nginx"]
...
@bot.message_handler(func=lambda v: v.text == "🧹 Очистить ключи" and v.from_user.id == ADMIN_ID)
def cmd_clean_keys(m):
    cleaned = {}
    for cat in goods:
        before = len(goods[cat])
        goods[cat] = [clean_key(k) for k in goods[cat] if is_valid_key(k)]
        after = len(goods[cat])
        cleaned[cat] = before - after
    save_goods()
    msg = "🧹 Очищено:\n" + "\n".join(f"{c}: {n}" for c, n in cleaned.items())
