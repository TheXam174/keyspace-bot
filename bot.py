...
def is_valid_key(line):
    ...
    if len(line) > 200:
        return False
    bad = ["<html", "404", "not found", "error", "<script", "<!DOCTYPE", "Apache", "nginx", "Server at"]
...
def cmd_buy(m):
    text = m.text
    for cat in prices:
        if text.startswith(f"🛍 Купить {cat}"):
            if not goods.get(cat):
                return bot.reply_to(m, "Нет доступных ключей.")
            ...
            if link:
                ...
                bot.send_message(m.chat.id, "Ошибка при создании платежа.")
            return
    bot.send_message(m.chat.id, "Категория не найдена.")

@bot.message_handler(func=lambda v: v.text.startswith("📅 Подписка "))
def cmd_sub(m):
    text = m.text
    for cat in sub_prices:
        if text.startswith(f"📅 Подписка {cat}"):
            link = create_payment_link(sub_prices[cat], f"Подписка {cat}", "https://t.me/ULBA_bot", m.from_user.id, cat, subscription=True)
            if link:
                bot.send_message(m.chat.id, f"Оформить подписку на {cat} за {sub_prices[cat]}₽/мес", reply_markup=get_payment_markup(cat, link))
            else:
                bot.send_message(m.chat.id, "Ошибка при создании подписки.")
            return
    bot.send_message(m.chat.id, "Категория подписки не найдена.")
