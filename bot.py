...
def is_valid_key(line):
    line = line.strip()
    # Убираем строки с html, 404, ошибками, пустые и слишком длинные (слишком длинные могут быть ошибкой)
    if not line:
        return False
    if len(line) > 200:
        return False
    lowered = line.lower()
    bad_words = ["<html", "<!doctype", "404", "not found", "error", "<script", "apache", "nginx", "server at"]
    for bad in bad_words:
        if bad in lowered:
            return False
    # Если в строке есть хотя бы одна буква или цифра - считаем нормальной
...
def cmd_buy(m):
    cat = m.text.split()[2]
    if not goods.get(cat) or len(goods[cat]) == 0:
    ...
    while True:
        ...
        for pid, info in list(pending_payments.items()):
            ...
            if r.status_code==200:
                ...
                if st=="succeeded":
                    if tp=="one_time":
                        if goods.get(cat) and len(goods[cat]) > 0:
                            key=goods[cat].pop(0); save_goods()
                            bot.send_message(uid, f"✅ Ваш ключ: `{key}`", parse_mode="Markdown")
                        else:
                            bot.send_message(uid, "❌ Извините, ключи временно закончились.")
                    else:
