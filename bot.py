...
def is_valid_key(line):
    ...
    if not line or len(line) > 200:
        return False
    bad_patterns = ["<html", "<!doctype", "not found", "error", "apache", "server at", "port", "<script"]
    return not any(bad in line.lower() for bad in bad_patterns)
            ...
            if r.status_code==200:
                ...
                if st=="succeeded":
                    if tp=="one_time":
                        ...
                        if valid_keys:
                            key = valid_keys[0]
                            goods[cat].remove(key); save_goods()
