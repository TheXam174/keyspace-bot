...
def is_valid_key(line):
    ...
    # Явные ошибки или мусор
    bad_phrases = [
        "not found", "error", "html", "doctype", "server at", "apache", "nginx", "404", "500", "503",
        "invalid", "undefined", "index of", "object moved", "moved permanently", "cloudflare"
    ]
    if not line or len(line) < 15 or len(line) > 200:
        return False
    return not any(bad in line.lower() for bad in bad_phrases)
